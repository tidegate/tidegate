...
func main() {
	...
	generator, err := core.NewLetsEncryptCertificateGenerator("/tmp/certs", "https://acme-v01.api.letsencrypt.org/directory", 81, 444, 2048)
	if err != nil {
		logger.Fatalf("%s", err)
	}
	certmgr := core.NewCertificateManager(generator)
	certLoader := core.NewFileCertificateLoader("/tmp/certs")
	_, err = certmgr.LoadCertificates(certLoader)

	dockerMgr, err := monitors.NewDockerManager(args.DockerDaemonAddr)

	if err == nil {
		serverManager := core.NewServerManager(certmgr)
