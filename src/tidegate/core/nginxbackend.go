...
func NewNGINXRPConfigurationGenerator(outputDirPath string) (res *NGINXRPConfigurationGenerator, err error) {
	res = &NGINXRPConfigurationGenerator{}

	absOutputDirPath, err := filepath.Abs(outputDirPath)
	if err != nil {
		err = errors.New(fmt.Sprintf("Invalid output directory path '%s': Unable to compute absolute path", outputDirPath))
		return
	}
	...
	serverPath := filepath.Join(res.configDirPath, "servers")
	err = os.MkdirAll(serverPath, 0700)
	if err == nil {
		RootLogger.Debugf("Created servers directory '%s'", serverPath)
	} else {
		err = errors.New(fmt.Sprintf("Unable to create servers directory '%s': %s", serverPath, err.Error()))
		return
	}
...
func (self NGINXRPConfigurationGenerator) GenerateConfiguration(server *Server) (err error) {
...
{{end}}
  ...
  if err == nil {
    serverConfigFilePath := filepath.Join(self.configDirPath, "servers", server.Domain)
  	fi, err := os.Create(serverConfigFilePath)
  	if err == nil {
  		err = t.Execute(fi, *server)
  	} else {
  		err = errors.New(fmt.Sprintf("Unable to create server configuration path '%s': %s", serverConfigFilePath, err.Error()))
  	} 
  } else {
...
func NewNGINXBackend(runDirPath string, binDirPath string) (res *NGINXBackend, err error) {
	configDirPath := filepath.Join(runDirPath, "config")

	res = &NGINXBackend{}
	res.configGenerator, err = NewNGINXRPConfigurationGenerator(runDirPath)

	res.daemon, err = NewNGINXDaemon(filepath.Join(configDirPath, "nginx.conf"), filepath.Join(binDirPath, "nginx"))
	if err == nil {
...
func (self NGINXBackend) HandleEndpointCreation(server *Server) (err error) {
	err = self.configGenerator.GenerateConfiguration(server)
	if(self.daemon.Status()) {
	  self.daemon.Reload()
	}
	return
}
func (self NGINXBackend) HandleEndpointDeletion(server *Server) (err error) {
